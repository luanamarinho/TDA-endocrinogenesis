...
def bastidas_pontes_pipeline(
    ...
    **kwargs,
    ...
    prep.qc_filter(
        adata, min_genes=min_genes, min_cells=min_cells, max_mt_perc=max_mt_perc
    )
    ...
    embedding_collection = prep.dimensionality_reduction(
        data=adata.X,
        method=dr_method,
        metric=umap_metric,
    ...
    adata_metadata = {
        "clusters_coarse": adata.obs["clusters_coarse"],
        "clusters": adata.obs["clusters"],
    }

    return {
        "embedding_collection": embedding_collection,
        "adata_collection": adata_collection,
...
def main():
    parser = argparse.ArgumentParser(
        description="Preprocess scVelo pancreas dataset and compute UMAP."
    )
